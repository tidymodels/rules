...
Also, both the predictors and binary rules are passed to the `glmnet` function. Since this model penalizes using the sum of the absolute coefficients, the predictors should be on the same scale. Otherwise, predictors with very large ranges will have an undue influence on the model. To do this, a recipe can be used to scale the original (raw) predictors to be between zero and one so that they are on the same scale as the rules: 
...
rule_fit_rec <- 
  ...
  step_range(starts_with("bill_"), flipper_length_mm)
...
rule_fit_wflow <- 
  ...
  add_recipe(rule_fit_rec)

rule_fit_fit <- fit(rule_fit_wflow, data = penguins)
```

```{r rf-penguins-print}
rule_fit_fit %>% pull_workflow_fit()
```
...
```r
( bill_depth_mm >= 0.125 ) & 
( flipper_length_mm <  0.957627118 ) & 
( flipper_length_mm >= 0.432203412 ) & 
( flipper_length_mm >= 0.88983047 ) & 
( flipper_length_mm >= 0.932203412 )
```
...
A single rule might be represented with multiple rows of this version of the data. These results can also be used to compute a rough estimate or variable importance using the absolute value of the coefficients (since the model predictors were scaled to be between zero and one): 
